...
\text 此階段的理論基礎聚焦於克服僅依賴原始OpenAPI規範作為測試輸入的侷限性，特別是解決黑箱測試中長期存在的測試腳本問題（Test Oracle Problem）。傳統的REST API模糊測試（Fuzzing）方法，其挑戰在於生成的請求序列普遍缺乏業務語義價值，從而導致大量的語義缺失請求在雲端閘道層被語法或業務邏輯檢查所阻擋，嚴重影響測試效率。
...
\text 本階段的實施流程旨在將非結構化或半結構化輸入轉化為高語義的結構化上下文，為確保跨代理與工具間的語義一致性，本研究對 Manual Document（包含預期結果及操作步驟等）的依賴是一種經過深思熟慮的技術權衡，用以確保測試的語義一致性與回傳正確性。在複雜的RESTful API系統中，傳統方法難以在缺乏形式化斷言的情況下自動判定系統的端到端（End-to-end）輸出是否正確。輸入內容包括OpenAPI JSON檔案（定義API結構與參數名稱）和Manual Document（包含高語義測試案例、預期結果及操作步驟等），其中的人類預期結果（Human Expected Results）為 Analyzer Agent 提供了核心測試案例，這是驗證系統功能性，確保測試結果正確性所必需的黃金標準。同時，該文件也提供了高語義測試案例的初始骨架，確保了LLM後續擴增的數據具備語義真實性。
\text 為了緩解人工維護帶來的成本與「知識漂移」的結構性矛盾，本研究利用LLM的能力實現自動化語義增強。Planner Agent作為FastMCP Client，隨即透過A2A協定發出任務，並作為FastMCP Client，呼叫FastMCP服務（由 LangChain 程式庫支持的工具互動層），以執行API item的資料處理與分類，實現對規格數據和人類知識的安全且一致性存取。
\text 在FastMCP服務內部，嵌入的LLM-Based Semantic Optimization組執行語義增強演算法：

\begin{enumerate}
    \item 模組透過精確的Prompt Engineering，指示底層 LLM 學習原始規範中參數的潛在語義（例如，從名稱判斷用途），並生成多個語義相似且具代表性的參數值集合（RTSet）。
    \item 緊接著，LLM自動掃描原始OpenAPI規範，將生成的para-value鍵值對從 RTSet 替換或補充到對應參數的enum字段中。此步驟旨在確保Test Coder Agent能夠存取到具備業務語義的有效輸入值，而非隨機生成的無效數據。
\end{enumerate}
...
為實現上述目標，本研究提出之「雙協定混合架構框架」之架構圖，如下圖3.1所示。
\begin{figure*}[htbp]
    \centering
