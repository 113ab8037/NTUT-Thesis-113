...
\text 進一步地，隨著資訊技術（IT）自動化的演進，基於LLM的自主代理（AI Agents）系統已成為新一代智能測試與運維自動化的關鍵推手。\cite{Hou2025MCP}\cite{Bandi2025Rise}AI Agent（AI 代理）自主的軟體實體，它感知環境（例如使用者查詢、感測器數據）、推理並採取行動（例如使用者查詢、感測器數據）來完成特定任務或目標。\cite{Bandi2025Rise}然而，這個術語常被稀釋，用來描述缺乏自主性、持久記憶或真正協調能力的組件，例如LangChain和AutoGen某些簡單的流程，或OpenAI Cookbook中的硬編碼腳本，常被誤稱為「代理」。\cite{Borghoff2025Beyond}相較之下，Agentic AI（代理式 AI）多代理系統（Multi-Agent Systems, MAS），其中多個專業代理協作、協調並規劃，以實現複雜、高層次的目標。Agentic AI 是一種自主的、目標驅動的系統，能夠在極少人類監督下，長時間獨立運作。\cite{Bandi2025Rise}模組化多代理系統透過明確定義的協議進行協調，能夠執行複雜的、多步驟的任務。\cite{Borghoff2025Beyond}
...
\text 在這個多代理協作過程中，代理人間協定（Agent-to-Agent Protocol, A2A）的作用至關重要。A2A以標準化的資料結構管理代理之間的狀態傳遞，使任務委派過程具一致性，以實現協作式任務執行。隨後，系統將這些狀態物件完整傳遞至LangChain Agent，作為上下文提供給MCP的工具調用機制。藉由A2A與MCP的混合架構（Hybrid Architecture），本研究實現了代理間的水平協作溝通（A2A）與模型對工具使用的垂直整合（MCP）的明確分工，有效解決在複雜測試流程中的任務協調成本與上下文一致性問題。此架構的核心價值在於建構一個安全、可擴展且可互通的代理式AI生態系統，有效地將LLM的語義推理能力與形式化方法的協調優勢結合，以應對現代軟體測試的挑戰。\cite{Borghoff2025Beyond}

\section{研究目標}

本研究主要解決當前軟體測試領域在RESTful API自動化測試整體的效率問題，提出並驗證一套有效可實際落地的RESTful API黑箱測試自動化流程的整合框架，將Agent-to-Agent協定（A2A）與Model Context Protocol（MCP）結合，透過混合式多代理架構，明確劃分了代理間的溝通與工具的使用，並在測試時自動化驗證，最終目標在於提升自動化測試的整體正確性，以降低維護成本及回饋週期。
